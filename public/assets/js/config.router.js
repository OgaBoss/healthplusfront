 ...
 */
app.config(['$authProvider','$stateProvider', '$urlRouterProvider', '$controllerProvider', '$compileProvider', '$filterProvider', '$provide', '$ocLazyLoadProvider', '$httpProvider','JS_REQUIRES','$injector',
    function ($authProvider, $stateProvider, $urlRouterProvider, $controllerProvider, $compileProvider, $filterProvider, $provide, $ocLazyLoadProvider,$httpProvider,jsRequires, $injector) {
            ...
            return {
                ...
                'responseError': function (response) {
                    if (response.data.error == 'token_not_provided' || response.data.error == 'token_invalid' || response.data.error == 'token_expired') {
                        var healthNotify = $injector.get("healthNotify");
                        $location.path('/login');
                        $localStorage.$reset();
                        healthNotify.set('Your session has expired, Please login again', 'error');
                    }
